...
// $api_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiQnJhbmQiLCJuYW1lIjoiQnJhbmQiLCJBUElfVElNRSI6MTcyOTc5NDEyMH0._KSQf4b4fYvX7ouTNe7WFNl6wYCEB6U2veSRg90PzL0";
// $api_base_url = 'https://dashboard.techmefi.com/api/';
$items = [
    [
...
if ($invoice_info->status) {
    $invoice_url = "https://dashboard.techmefi.com/invoice/" . $invoice_info->invoice->id . "/" . $invoice_info->invoice->hash;
    // echo json_encode([
    //     'status' => true,
    //     'message' => "Success",
    //     'url' => $invoice_url,
    ...
    // CATCH ERRORS
    $payment_page_url = "https://techmefi.com/checkout.php";
    // echo json_encode([
    //     'status' => false,
...
{
    $url = "https://dashboard.techmefi.com/api/leads/" . $lead_id;
    $ch = curl_init();
...
{
    $url = "https://dashboard.techmefi.com/api/customers";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
...
{
    $url = "https://dashboard.techmefi.com/api/contacts";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
{
    $url = "https://dashboard.techmefi.com/api/invoices";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
